...
def render(ctx: dict):
    ...
    with st.expander("Advanced Weekly Detail Settings", expanded=False):
        ...
        with adv3:
            avg_window_label = st.selectbox(
                "Average Window",
                options=["None", "4 weeks", "8 weeks", "13 weeks", "26 weeks"],
                index=1,
    ...
    if d_all.empty:
        ...
        wk_sales = d_all.groupby([pivot_dim, "WeekEnd"], as_index=False).agg(Sales=("Sales", "sum"))
        wk_sales["WeekEnd"] = pd.to_datetime(wk_sales["WeekEnd"], errors="coerce")
        wk_sales = wk_sales[wk_sales["WeekEnd"].notna()].copy()

        all_weeks = sorted(pd.to_datetime(wk_sales["WeekEnd"].dropna().unique()).tolist())

        def _parse_week_count(label: str):
            if label == "All weeks":
                return None
            return int(str(label).split()[0])

        display_n = _parse_week_count(display_timeframe)
        ...
        wk_sales_disp = wk_sales[wk_sales["WeekEnd"].isin(display_weeks_set)].copy()
        wk_sales_disp["Week"] = wk_sales_disp["WeekEnd"].dt.date.astype(str)

        piv = wk_sales_disp.pivot_table(
            index=pivot_dim,
            columns="Week",
            values="Sales",
            aggfunc="sum",
        ...
        row_order = sorted(wk_sales_disp[pivot_dim].dropna().unique().tolist())
        ...
        if avg_n is None:
            ...
            if avg_existing_cols:
                ...
                numeric["Δ vs Avg"] = (
                    numeric[current_col] - numeric["Average"] if current_col is not None else np.nan
                )
            else:
                numeric["Average"] = np.nan
                numeric["Δ vs Avg"] = np.nan
        ...
        for c in existing_week_cols:
            display_df[c] = display_df[c].map(money)

        display_df["Δ vs prior week"] = display_df["Δ vs prior week"].map(
            lambda x: "" if pd.isna(x) else f"{'+' if x > 0 else ''}{money(x)}"
        )

        if "Average" in display_df.columns:
            display_df["Average"] = display_df["Average"].map(lambda x: "" if pd.isna(x) else money(x))

        if "Δ vs Avg" in display_df.columns:
            display_df["Δ vs Avg"] = display_df["Δ vs Avg"].map(
                lambda x: "" if pd.isna(x) else f"{'+' if x > 0 else ''}{money(x)}"
            )
        ...
        delta_cols = ["Δ vs prior week"]
        if "Δ vs Avg" in display_df.columns:
            delta_cols.append("Δ vs Avg")

        styled_weekly = _style_delta_cols(display_df, numeric_reset, delta_cols)
    ...
    if life.empty:
        ...
        with right:
            ...
            cols = [
                c for c in [
                    "SKU", "Stage", "Trend", "Momentum", "Sales (lookback)", "Units (lookback)",
                    "Last Week Sales", "WoW Sales Δ", "Slope", "Weeks Up", "Weeks Down",
