...
def run_app():
    ...
    if scope == "Retailer" and scope_pick: df_scope = df_scope[df_scope["Retailer"].isin(scope_pick)]
    elif scope == "Vendor" and scope_pick: df_scope = df_scope[df_scope["Vendor"].isin(scope_pick)]
    elif scope == "SKU" and scope_pick: df_scope = df_scope[df_scope["SKU"].isin(scope_pick)]
    ...
    if analysis_view in ("Month / Year Compare", "Multi Month / Year Compare"):
        ...
        pA = period_from_df(dfA); pB = period_from_df(dfB) if not dfB.empty else None
        if pA is None:
        ...
        if compare_mode == "None": pB = None; dfB = dfA.iloc[0:0].copy()
        elif compare_mode.startswith("Prior"): pB = period_prev_same_length(pA); dfB = filter_by_period(df_scope, pB)
        else: pB = period_yoy(pA); dfB = filter_by_period(df_scope, pB)
    ...
    if compare_mode != "None" and pB is not None: st.sidebar.markdown(f"**Compare:** {b_lbl}<br><span style='opacity:0.8'>{format_period_range(pB)}</span>", unsafe_allow_html=True)
    else: st.sidebar.markdown("<span style='opacity:0.75'>Compare: None</span>", unsafe_allow_html=True)
    st.sidebar.divider()

    kA = calc_kpis(dfA); kB = calc_kpis(dfB) if pB is not None else {k:0.0 for k in kA.keys()}
    df_hist_for_new = df_all.copy()
    if scope == "Retailer" and scope_pick: df_hist_for_new = df_hist_for_new[df_hist_for_new["Retailer"].isin(scope_pick)]
    elif scope == "Vendor" and scope_pick: df_hist_for_new = df_hist_for_new[df_hist_for_new["Vendor"].isin(scope_pick)]
    elif scope == "SKU" and scope_pick: df_hist_for_new = df_hist_for_new[df_hist_for_new["SKU"].isin(scope_pick)]

    cur_range = format_period_range(pA); cmp_range = format_period_range(pB) if pB is not None else ""; cmp_name = b_lbl or ""
    st.markdown(f"""<div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; margin: 4px 0 10px 0;"><span style="padding:6px 10px; border-radius:999px; border:1px solid rgba(128,128,128,0.22); background: var(--secondary-background-color); color: var(--text-color); font-weight:700; font-size:12px;">Scope: {scope}</span><span style="padding:6px 10px; border-radius:999px; border:1px solid rgba(128,128,128,0.22); background: var(--secondary-background-color); color: var(--text-color); font-weight:700; font-size:12px;">Current: {a_lbl} • {cur_range}</span>{(f'<span style="padding:6px 10px; border-radius:999px; border:1px solid rgba(128,128,128,0.22); background: var(--secondary-background-color); color: var(--text-color); font-weight:700; font-size:12px;">Compare: {cmp_name} • {cmp_range}</span>' if compare_mode != "None" and pB is not None else '<span style="padding:6px 10px; border-radius:999px; border:1px solid rgba(128,128,128,0.22); background: var(--secondary-background-color); color: var(--text-color); font-weight:700; font-size:12px; opacity:0.75;">Compare: None</span>')}</div>""", unsafe_allow_html=True)
    st.write("")

    sales_delta = kA["Sales"] - kB.get("Sales", 0.0); units_delta = kA["Units"] - kB.get("Units", 0.0); asp_delta = kA["ASP"] - kB.get("ASP", 0.0)
    drv = drivers(dfA, dfB, driver_level)
    top_pos = drv[drv["Sales_Δ"] > 0].head(1); top_neg = drv[drv["Sales_Δ"] < 0].tail(1)
    headline_bits=[]
    if compare_mode != "None":
        headline_bits.append(f"Sales {'up' if sales_delta >= 0 else 'down'} **{money(abs(sales_delta))}** vs comparison.")
        headline_bits.append(f"Units: **{kA['Units']:,.0f}** ({'up' if units_delta >= 0 else 'down'} **{abs(units_delta):,.0f}** vs comparison).")
        headline_bits.append(f"ASP: **{money(kA['ASP'])}** ({'up' if asp_delta >= 0 else 'down'} **{money(abs(asp_delta))}** vs comparison).")
        if not top_pos.empty: headline_bits.append(f"Top driver: **{top_pos.iloc[0][driver_level]}** ({money(float(top_pos.iloc[0]['Sales_Δ']))}).")
        if not top_neg.empty: headline_bits.append(f"Top drag: **{top_neg.iloc[0][driver_level]}** ({money(float(top_neg.iloc[0]['Sales_Δ']))}).")
    else: headline_bits.append("Choose a comparison mode to see drivers and deltas.")
    def _md_bold_to_html(s: str) -> str:
        parts=[]; last=0
        for mm in _re.finditer(r"\*\*(.+?)\*\*", s):
            parts.append(_html.escape(s[last:mm.start()])); parts.append(f"<strong>{_html.escape(mm.group(1))}</strong>"); last=mm.end()
        parts.append(_html.escape(s[last:])); return ''.join(parts)
    _lis = ''.join([f"<li>{_md_bold_to_html(x)}</li>" for x in headline_bits])
    st.markdown(f"<div class='intel-card'><div class='intel-header'>INTELLIGENCE SUMMARY</div><div class='intel-body'><ul>{_lis}</ul></div></div>", unsafe_allow_html=True)

    ctx = dict(dfA=dfA, dfB=dfB, kA=kA, kB=kB, a_lbl=a_lbl, b_lbl=b_lbl, compare_mode=compare_mode, min_sales=min_sales, min_units=min_units, driver_level=driver_level, df_scope=df_scope, pA=pA, pB=pB, df_hist_for_new=df_hist_for_new, show_full_history_lifecycle=show_full_history_lifecycle, analysis_view=analysis_view)
