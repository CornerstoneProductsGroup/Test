...
def run_app():
    ...
    with st.sidebar:
        ...
        if scope == "Retailer":
            scope_pick = st.multiselect(
                "Retailer(s)",
                options=sorted(df_all["Retailer"].dropna().unique()),
                default=[],
            )
        elif scope == "Vendor":
            scope_pick = st.multiselect(
                "Vendor(s)",
                options=sorted(df_all["Vendor"].dropna().unique()),
                default=[],
            )
        elif scope == "SKU":
            scope_pick = st.multiselect(
                "SKU(s)",
                options=sorted(df_all["SKU"].dropna().unique()),
                default=[],
            )
    ...
    if analysis_view == "Multi Month / Year Compare":
        ...
        top_neg = drv[drv["Sales_Δ"] < 0].tail(1)
