...
# Require at least 2 chars and at least one digit; allow -, /, _, .
...
def _seq_match(words_lower, i, seq_tokens):
    ...
    for need in seq_tokens[1:]:
        ...
        for _, txt2, w2 in words_lower:
            w2_base = (w2.get('top', 0) + w2.get('bottom', 0)) / 2.0
            if abs(w2_base - baseline) <= 4.0 and w2.get('x0', 0) >= cur_x - 2 and (w2.get('x0', 0) - cur_x) <= 160:
                if txt2 == need:
                    nxt = w2
                    break
        if nxt is None:
...
def _same_baseline(w1, w2, tol=4.0):
    b1 = (w1.get('top', 0) + w1.get('bottom', 0)) / 2.0
    b2 = (w2.get('top', 0) + w2.get('bottom', 0)) / 2.0
    return abs(b1 - b2) <= tol

def _find_vendor_pn_column_band(words, x_pad_left=14, x_pad_right=44, below_px=90):
    """Find the 'Vendor PN' header and return a vertical capture band below that column."""
...
def _find_vendor_itemnum_band(words, x_pad_left=18, x_pad_right=48, below_px=100):
    """Find the 'Vendors (Sellers) Item Number:' label (many variants) and return a narrow capture band below it."""
    lower = [(i, (w.get('text', '') or '').strip().lower(), w) for i, w in enumerate(words)]
    variants = [
...
def split_pdf_to_vendors(pdf_path: str, out_dir: str, vendor_map: Dict[str, str], threshold=0.88):
    ...
    report_rows = []
    review_rows = []
    page_exports: Dict[str, List[int]] = {}
